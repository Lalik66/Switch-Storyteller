...
- [ ] **PR #2 targeted safety re-read (follow-up, NOT started)** — the community audit found four safety issues in PR #2's work, and PR #2's merge also carried the entire Phase 2 (`character`/`story_image`/`story_audio`/`parent_report`) and Phase 4 (`user_role` + reviewer columns) schema that was never reviewed against the plan. Do a targeted read of PR #2's **Phase 2 and Phase 4 diffs** specifically, for further issues in the parts that shipped unreviewed. Owner-flagged 2026-07-17; not urgent.
